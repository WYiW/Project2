...
class FracNetTrainDataset(Dataset):
    ...
    def __init__(self, image_dir, label_dir=None, crop_size=64,
            transforms=None, num_samples=2, train=True):
    ...
    def _get_spine_neg_centroids(shape, crop_size, num_samples):
        x_min, x_max = crop_size // 2, shape[0] - crop_size // 2
        y_min, y_max = crop_size // 2, shape[1] - crop_size // 2
        z_min, z_max = min(crop_size // 2, shape[2] - crop_size // 2)-1,max(crop_size // 2, shape[2] - crop_size // 2)+1
        spine_neg_centroids = [(

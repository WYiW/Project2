...
# target one-hot编码
def to_one_hot_3d(tensor, n_classes=1):  # shape = [batch, s, h, w]
    n, c, s, h, w = tensor.size()
    one_hot = torch.zeros(n, n_classes, s, h, w).scatter_(1, tensor.view(n, 1, s, h, w), 1)
    # test1=np.array(one_hot[0][0])
    # test2=np.array(one_hot[0][1])
    # testsum=test1+test2
    # one_hot_txt=np.short(np.reshape(one_hot,(one_hot.shape[0],-1)))
    # np.savetxt("one_hot_label_2.csv",one_hot_txt)
    return one_hot

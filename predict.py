...
from fracnet_dataset import FracNetInferenceDataset
...
# def _remove_low_intensities(pred, intensity_thresh):
#     pred_label = label(pred > 0).astype(np.int16)
#     for region in regionprops(pred):
#         if region.mean_intensity < intensity_thresh:
#             region.label = np.intenger(0)
    ...
    with torch.no_grad():
        for _, sample in enumerate(dataloader):
            ...
            for i in range(len(centers)):
                ...
                choose_pred=np.where(cur_pred_patch > 0, np.mean((output[i][1],cur_pred_patch), axis=0), output[i][1])
...
def _make_submission_files(pred, image_id, affine, intensity_thresh):
    pred_label = label(pred > 0).astype(np.int16)
    ...
    for region in pred_regions:
        ...
        if region.mean_intensity < intensity_thresh:
            region.label = 0
            pred_label[r_slice] = np.where(pred_label[r_slice]==r_label,0,pred_label[r_slice])
        else:
            new_label = new_label + 1
            region.label = new_label
            pred_label[r_slice] = np.where(pred_label[r_slice]==r_label,new_label,pred_label[r_slice])
 
    pred_index = [0] + [region.label for region in pred_regions if region.label > 0]
    pred_proba = [0.0] + [region.mean_intensity for region in pred_regions if region.label > 0]
    # placeholder for label class since classifaction isn't included
    pred_label_code = [0] + [1 for region in pred_regions if region.label > 0]
    pred_image = nib.Nifti1Image(pred_label, affine)
    # print(len(pred_index),len(pred_proba),len(pred_label_code))
...
def predict(args):
    ...
    if args.model_path is not None:
        model_weights = torch.load(args.model_path,map_location=torch.device('cpu'))['net']
    ...
    for image_id, image_path in zip(image_id_list, image_path_list):
        ...
        pred_image, pred_info = _make_submission_files(pred_arr, image_id,
                                                       dataset.image_affine, args.intensity_thresh)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_dir", default="data/test/image",
                        help="The image nii directory.")
    parser.add_argument("--pred_dir", default="data/test/pred",
                        help="The directory for saving predictions.")
    parser.add_argument("--model_path", default="experiments/ResUNet/best_model.pth",
                        help="The PyTorch model weight path.")
    parser.add_argument("--prob_thresh", default=0.64,
                        help="Prediction probability threshold.")
    parser.add_argument("--bone_thresh", default=300,
                        help="Bone binarization threshold.")
    parser.add_argument("--size_thresh", default=150,
                        help="Prediction size threshold.")
    parser.add_argument("--intensity_thresh", default=0.71,
                        help="Prediction mean intensity threshold.")
    parser.add_argument("--postprocess", default="True",
                        help="Whether to execute post-processing.")
    args = parser.parse_args()

    with concurrent.futures.ProcessPoolExecutor() as executor:
        executor.map(predict(args))
